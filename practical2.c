...
struct member* reverse(struct member *);
void concat(struct member*);
...
void concat(struct member *head) {
    struct member *newDiv = init();
...
struct member* reverse(struct member *ptr) {
    if(ptr == NULL) {
        return NULL;
    }
    if(ptr->next == NULL) {
...
int main()
{
    printf("\tWelcome to COMET GHRCEM\n");
